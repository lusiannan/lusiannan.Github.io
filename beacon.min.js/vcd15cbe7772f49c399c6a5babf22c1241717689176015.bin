...
(function() {
    ...
    // 2. Anti-Developer Tools
    function detectDevTools() {
        var element = new Image();
        Object.defineProperty(element, 'id', {
            get: function() {
                // Redirect atau aksi ketika dev tools terbuka
    ...
    setInterval(function() {
        ...
        if (currentTime - startTime > 100) {
            // Debugger detected - redirect or clear
    ...
    // 6. Script Integrity Check
    var originalScripts = ['beacon.min.js']; // Ganti dengan script Anda
    setInterval(function() {
        var scripts = document.querySelectorAll('script[src]');
        scripts.forEach(function(script) {
            var src = script.src;
            if (src && !originalScripts.some(function(orig) {
                return src.includes(orig);
            })) {
                script.remove();
                console.warn('Unauthorized script removed:', src);
            }
        });
    }, 2000);
    
    // 7. Execution Environment Check
    if (window.console && console.log) {
        var originalLog = console.log;
        console.log = function() {
            // Block sensitive information logging
            var args = Array.prototype.slice.call(arguments);
            if (args.some(function(arg) {
                return typeof arg === 'string' && 
                      (arg.includes('token') || arg.includes('key') || arg.includes('secret'));
            })) {
                return;
            }
